...
export const createProgram = async (req, res) =>
{
    ...
    let { name, focusBodyPart, intensity, difficulty, targetAudience, reps, isActive } = programData;

    isActive = isActive ?? true;

    // Change to lower case 
    [name, focusBodyPart, intensity, difficulty, targetAudience] =
        [name, focusBodyPart, intensity, difficulty, targetAudience].map(str => str.toLowerCase());
    ...
    } else if (difficulty === "intermediate" && intensity === "high" && reps > 20) {
